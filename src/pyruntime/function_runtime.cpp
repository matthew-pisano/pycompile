...
PyValue* pyir_makeFunction(void* fn_ptr) { return new PyValue(reinterpret_cast<PyFunction>(fn_ptr)); }


PyValue* pyir_call(const PyValue* callee, PyValue** args, const int64_t argc) {
    if (callee->isFn()) {
        ValueRef result(std::get<PyFunction>(callee->data)(args, argc));
        return result.release();
    }
    if (callee->isBoundMethod()) {
        const PyBoundMethod& bm = std::get<PyBoundMethod>(callee->data);
